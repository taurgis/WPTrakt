...
namespace Delay
{
    ...
    public static class LowProfileImageLoader
    {
        ...
        private static WriteableBitmap ResizeWriteableBitmap(WriteableBitmap bitmap, double newWidth, double newHeight, Image tempImage)
        {
            try
            {
                WriteableBitmap resize = new WriteableBitmap((int)newWidth, (int)newHeight);
                ScaleTransform transform = new ScaleTransform();
                transform.ScaleX = newWidth / bitmap.PixelWidth;
                transform.ScaleY = newHeight / bitmap.PixelHeight;
                resize.Render(tempImage, transform);
                resize.Invalidate();
                return resize;
            }
            catch (Exception ex)
