...
namespace WPtrakt
{
    public class MovieViewModel : INotifyPropertyChanged
    {
        ...
        {
            String fileName = this.Imdb + "background." + ImageController.getExtentionFromUrl(this.Fanart);

            if (StorageController.doesFileExist(fileName))
            {
                _backgroundImage = ImageController.loadImageFromStream(this.Imdb + "background", ImageController.getExtentionFromUrl(this.Fanart), 800, 600);
        ...
        void request_OpenReadFanartCompleted(IAsyncResult r)
        {
            try
            {
                ...
                if (status == System.Net.HttpStatusCode.OK)
                {
                    ...
                    ImageController.saveImageAsStream(this.Imdb + "background", ImageController.getExtentionFromUrl(this.Fanart), str);
