...
namespace WPtrakt
{
    public partial class ViewShow : PhoneApplicationPage
    {
        ...
        {
            App.TrackEvent("ViewShow", "Switched panorama to " + this.ShowPanorama.SelectedIndex);
            lastIndex = this.ShowPanorama.SelectedIndex;
            RefreshBottomBar();
        }
        ...
        private void RefreshBottomBar()
        {
            ...
            {
                if (App.ShowViewModel.RelatedShows.Count == 0)
                    this.LoadRelatedShows();

                this.ApplicationBar = null;
            }
            else if (this.ShowPanorama.SelectedIndex == 4)
            {
        ...
        private async void LoadRelatedShows()
        {
            ...
            {
                ListItemViewModel newModel = new ListItemViewModel() { ImageSource = relatedShow.Images.Poster, Tvdb = relatedShow.tvdb_id, Type = "show" };
        ...
        private async void refreshButton_Click(object sender, EventArgs e)
        {
            if (!this.LoadingActive)
            {
                App.TrackEvent("ViewShow", "Refresh");
                this.LoadingActive = true;
        ...
        {
            App.TrackEvent("ViewShow", "Rate");
            NavigationService.Navigate(new Uri("/RatingSelector.xaml?type=show&imdb=" + App.ShowViewModel.Imdb + "&year=" + App.ShowViewModel.Year + "&title=" + App.ShowViewModel.Name + "&tvdb=" + App.ShowViewModel.Tvdb, UriKind.Relative));
        }
        ...
        private void ApplicationBarIconButton_Click_EpisodeBack(object sender, EventArgs e)
        {
            if (!this.LoadingActive)
            {
                ...
                App.TrackEvent("ViewShow", "Previous season");
        ...
        private void ApplicationBarIconButton_Click_EpisodeForward(object sender, EventArgs e)
        {
            if (!this.LoadingActive)
            {
                ...
                App.TrackEvent("ViewShow", "Next season");
        ...
        {
            this.progressBarLoading.Visibility = System.Windows.Visibility.Visible;
            App.TrackEvent("ViewShow", "Watchlist context");
            lastModel = (ListItemViewModel)((MenuItem)sender).DataContext;
